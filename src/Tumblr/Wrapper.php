...
class Wrapper {
	...
	protected $baseUrl = 'http://api.tumblr.com/v2';
	protected $sandbox;
	...
	protected $authorizationUrl = 'http://www.tumblr.com/oauth/authorize?oauth_token=%s';
	...
	public function __construct($baseName, $consumerKey, $consumerSecret) {
	...
	public function getClient($token = false, $tokenSecret = false) {
		if (!is_null($this->client)) {
			...
			$this->client = new Client($this->baseUrl, array(
				'curl.timeout' => '1000'
			));
	...
	public function makeAuthUrl() {
		return sprintf($this->authorizationUrl, urlencode($this->tokens['oauth_token']));
	}
